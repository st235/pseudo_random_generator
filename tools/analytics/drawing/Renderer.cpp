...
Renderer::Renderer(std::string filename, std::string extension): _filename(std::move(filename)), _extension(std::move(extension)) {
}
...
void Renderer::drawBitmap(const Bitmap *bitmap) {
...
std::unique_ptr<Renderer> Renderer::forBitmap(const Bitmap *bitmap) {
    return std::make_unique<Renderer>(bitmap->getTitle(), EXT_BMP);
}

std::string Renderer::EXT_BMP = "bmp";

std::string Renderer::DELIMITER_DOT = ".";
